            ...
            <div key={`cond-${idx}`} className={`query-row ${item.enabled ? "" : "disabled"}`}>
              ...
              <div>
                {item.boolType === "sort" ? (
                  ...
                  <select
                    className="form-control"
                    value={item.operator}
                    onChange={(event) => onChangeCondition(idx, { operator: event.target.value })}
                  >
              ...
              <div>
                {item.boolType === "sort" ? (
                  ...
                  <ConfigProvider locale={i18nLanguage === "zh" ? zhCN : enUS}>
                    <RangePicker
                      showTime
                      size="small"
              ...
              <div className="flex-gap justify-end">
                ...
                <button className="btn btn-ghost btn-icon text-danger" onClick={() => onRemoveCondition(idx)} title={t("dataBrowser.deleteRow")}>−</button>
              </div>
            </div>
          ))}
        </div>
